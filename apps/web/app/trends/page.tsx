...
const LANG_COLORS = [
  '#37e39f', '#22c7f2', '#8b8cff', '#f5c400', '#f75bb6',
]

const MONTHS_OPTIONS = [3, 6, 9, 12]

const CHANGE_LABELS = {
  new:  { text: '새로 시작 🆕', color: 'border-emerald-500/30 bg-emerald-500/15 text-emerald-400' },
  gone: { text: '최근 없음',    color: 'border-white/10 bg-white/6 text-[var(--dashboard-muted)]' },
  up:   { text: '↑ 증가',       color: 'border-blue-500/30 bg-blue-500/15 text-blue-400' },
  down: { text: '↓ 감소',       color: 'border-orange-500/30 bg-orange-500/15 text-orange-400' },
}
        ...
        {data && !loading && (
          <>
            ...
            <div className="grid grid-cols-3 gap-4">
              {[
                ...
                <div key={item.label} className="glass-panel p-4 text-center">
                  <div className="text-xl font-bold text-[var(--dashboard-accent)]">{item.value}</div>
                  <div className="text-xs text-[var(--dashboard-muted)] mt-1">{item.label}</div>
                </div>
              ))}
            </div>
            ...
            <div className="glass-panel p-5">
              <p className="mb-4 text-[10px] font-bold uppercase tracking-[0.3em] text-[var(--dashboard-muted)]">
                ...
                {chartType === 'bar' ? (
                  <BarChart data={data.chartData} barSize={14}>
                    ...
                    {data.topLangs.map((lang, i) => (
                      <Bar key={lang} dataKey={lang} stackId="a" fill={LANG_COLORS[i % LANG_COLORS.length]}
                  ...
                  <LineChart data={data.chartData}>
                    ...
                    <Legend wrapperStyle={{ fontSize: 12, color: 'var(--dashboard-soft)' }} />
                    {data.topLangs.map((lang, i) => (
                      <Line key={lang} type="monotone" dataKey={lang}
                        stroke={LANG_COLORS[i % LANG_COLORS.length]} strokeWidth={2}
            ...
            <div className="glass-panel p-5">
              ...
              <ResponsiveContainer width="100%" height={160}>
                <BarChart data={data.chartData} barSize={28}>
                  ...
                  <YAxis tick={{ fill: 'var(--dashboard-soft)', fontSize: 11 }} allowDecimals={false} />
                  <Tooltip contentStyle={TOOLTIP_STYLE} />
                  <Bar dataKey="total" fill="var(--dashboard-accent)" radius={[4, 4, 0, 0]} name="총 커밋"
            ...
            <div className="glass-panel p-5">
              ...
              {data.changes.length === 0 ? (
                <p className="text-sm text-[var(--dashboard-muted)]">기간 내 언어 사용 패턴이 안정적으로 유지되고 있어요.</p>
              ) : (
                <div className="flex flex-wrap gap-2">
                  {data.changes.map((c, i) => {
                    const meta = CHANGE_LABELS[c.type]
                    return (
                      <span key={i} className={`rounded-full border px-3 py-1 text-xs font-semibold ${meta.color}`}>
                        {c.lang} · {meta.text}{c.percent ? ` ${c.percent}%` : ''}
                      </span>
                    )
                  })}
                </div>
